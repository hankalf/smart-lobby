...
(async () => {
  ...
  await req('POST', '/api/kiosk/signin', {
    full_name: 'John Doe 41', company: 'Board Co', phone: '415-268-7001',
  ...
  const visit = (await req('GET', '/api/admin/visits?limit=50')).data.find((v) => v.full_name === 'John Doe 41');
  await req('POST', '/api/kiosk/signout', { visit_id: visit.id });
  data = await (await anon(`/api/board/${key}/data`)).json();
  ok('they leave the on-site list', !data.onsite.some((p) => p.name === 'John Doe 41'));
  ok('…and appear under just signed out', data.left.some((p) => p.name === 'John Doe 41'),
