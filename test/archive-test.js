...
(async () => {
  ...
  let r = await req('POST', '/api/kiosk/signin', {
    full_name: 'John Doe 40', company: 'Archive Co', phone: '415-268-1001',
  ...
  ok('the list shows who it was', entry && entry.label === 'John Doe 40', entry && entry.label);
  ...
  ok('the visit is back under its own id', detail && detail.full_name === 'John Doe 40', JSON.stringify(detail).slice(0, 80));
  ...
  ok('their visits are gone from the live list',
    !(await liveVisits()).some((v) => v.full_name === 'John Doe 40'));
  ...
  ok('their visit came back with them', back.some((v) => v.full_name === 'John Doe 40'),
