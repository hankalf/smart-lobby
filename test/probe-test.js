...
(async () => {
  ...
  r = await req('POST', '/api/admin/staff', { name: 'John Doe', email: 't@x.test', active: 1 });
  const host = r.data;
  r = await req('POST', '/api/kiosk/signin', { full_name: 'John Doe 17', company: 'C', phone: '415-268-0501',
  ...
  ok('their visitor record still opens afterwards', r.status === 200 && r.data.full_name === 'John Doe 17',
  ...
  r = await req('POST', '/api/kiosk/signin', { full_name: 'John Doe 15', company: 'C', phone: '415-268-0502',
  ...
  for (const [label, value] of odd) {
    r = await req('POST', '/api/kiosk/signin', {
      full_name: label.includes('company') ? 'John Doe 22' : value,
